...
class CompanyController extends ApiController {
    ...
    */
	...
	public function get()
	{
		...
		{
			$q.= ' AND company_id = '.replace_quote($attr['company_id']);
		}
    ...
    public function get_by_site_id()
	{
		$attr = NULL;
		if (! empty($_GET)) $attr = $_GET;
        ...
        SELECT * 
        FROM ' . $this->table . ' c
        LEFT JOIN ms_site s USING(company_id) 
        WHERE 1';
		
		if (isset($attr['site_id']) && $attr['site_id'] != '') 
		{
			$q.= ' AND s.site_id = '.replace_quote($attr['site_id']);
		}
	...
	public function get_list()
	{
		...
		if (isset($attr['keyword']) && $attr['keyword'] != '') {
			$q.= ' AND ( ';
			$q.= ' company_id LIKE '.replace_quote($attr['keyword'],'like');
			$q.= ' OR company_name LIKE '.replace_quote($attr['keyword'],'like');
  ...
  //       }
	...
	public function get_list_dropdown()
	{
		...
		$q = 'SELECT company_id, company_name FROM ' . $this->table . ' 
