...
class DivisionController extends ApiController {
    ...
    */
    ...
    public $list_column = array('division_id', 'division_name', 'chamber_sync_flag', 'field_sync', 'status', 'created_at', 'created_by','created_ip','updated_at','updated_by','updated_ip');
	...
	public function get_list_status()
	{
		...
		// $log = ArticleModel::where('division_id',3)
	...
	public function get()
	{
		...
		if (isset($attr['division_id']) && $attr['division_id'] != '') {
			$q.= ' AND division_id = '.$attr['division_id'];
		}
	...
	public function get_list()
	{
		...
		if (isset($attr['keyword']) && $attr['keyword'] != '') {
			$q.= ' AND ( ';
			$q.= ' division_name LIKE '.replace_quote($attr['keyword'],'like');
			$q.= ')';
        }
	...
	public function get_list_dropdown()
	{
		...
		$q = 'SELECT division_id, division_name FROM ' . $this->table . ' 
