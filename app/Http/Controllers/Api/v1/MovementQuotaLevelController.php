...
class MovementQuotaLevelController extends ApiController {
    ...
    */
	...
	public function get_list()
	{
		...
		$q = 'SELECT mv_quota.movement_quota_level_id, mv_quota.site_id, mv_quota.transaction_id, mv_quota.qty, mv_quota.value, mv_quota.addt, mv_quota.balance_qty, mv_quota.balance_value, mv_quota.balance_addt, mv_quota.created_at, mv_quota.status, mv_quota.transaction_code,';
		$q.= ' COALESCE((SELECT u.user_code FROM ms_user u WHERE mv_quota.created_by = u.user_id), mv_quota.created_by) as creator,';
		$q.= ' COALESCE((SELECT u.user_code FROM ms_user u WHERE mv_quota.user_id = u.user_id), mv_quota.user_id) as user_id';
		$q.= ' FROM ' . $this->table . ' mv_quota WHERE 1';
		$q.= ' HAVING 1';
