...
class UserAttributeValueController extends ApiController {
    ...
    */
    ...
    public $list_column = array('user_attribute_value_id', 'user_id', 'attribute_id','attribute_value', 'chamber_sync_flag', 'field_sync','status', 'created_at', 'created_by','created_ip','updated_at','updated_by','updated_ip');
	...
	public function get()
	{
		...
		if (isset($attr['user_attribute_value_id']) && $attr['user_attribute_value_id'] != '') {
			$q.= ' AND user_attribute_value_id = '.$attr['user_attribute_value_id'];
        }
	...
	public function get_list()
	{
		...
		$q = '
		SELECT ua.user_attribute_value_id, ua.user_id, at.attribute_name , ua.attribute_id, ua.attribute_value, CONCAT(u.firstname," ",u.lastname) as fullname, u.user_code, u.email, ua.status
		FROM ' . $this->table . ' ua 
        LEFT JOIN ms_user u USING(user_id)
        LEFT JOIN ms_attribute at USING(attribute_id)
        WHERE 1
        HAVING 1';
		...
		if (isset($attr['user_attribute_value_id']) && $attr['user_attribute_value_id'] != '') {
			$q.= ' AND ua.user_attribute_value_id = '.$attr['user_attribute_value_id'];
        }
