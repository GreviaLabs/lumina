...
class ReasonTypeController extends ApiController {
    ...
    */
    ...
    public $list_column = array('reason_type_id','article_attribute_value_id','site_id','article','attribute','attribute_value','status', 'created_at', 'created_by','created_ip','updated_at','updated_by','updated_ip');
	...
	public function get_list()
	{
		...
		$q = '
        SELECT rt.reason_type_id reason_type_id, rt.article article, rt.attribute attribute, rt.attribute_value attribute_value , rt.site_id site_id, rt.status status, rt.chamber_sync_flag chamber_sync_flag, 
        rt.created_at created_at, rt.created_ip created_ip,
        rt.updated_at updated_at, rt.updated_ip updated_ip,';
        $q.= ' COALESCE((SELECT u.user_code FROM ms_user u WHERE rt.created_by = u.user_id), rt.created_by) as creator,';
		$q.= ' COALESCE((SELECT u.user_code FROM ms_user u WHERE rt.updated_by = u.user_id), rt.updated_by) as editor';
        $q.= ' FROM ' . $this->table . ' rt
        LEFT JOIN ms_article a ON rt.article = a.article AND rt.site_id = a.site_id
        WHERE 1';
	...
	public function get_list_dropdown()
	{
		...
		$q = '
        SELECT reason_type_id, article, attribute, attribute_value
        FROM ' . $this->table . ' 
