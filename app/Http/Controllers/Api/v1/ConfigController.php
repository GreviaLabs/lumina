...
class ConfigController extends ApiController {
    ...
    */
	...
	public function get()
	{
        ...
        if (isset($attr['config_name']) && $attr['config_name'] != '') {
			$q.= ' AND config_name = '.replace_quote($attr['config_name']);
        }
	...
	public function update_duplicate_single()
	{
		$put = $attr = $result = NULL;
		...
		if (! isset($attr['config_name'])) $result['message'] = 'config_name must be filled.';
		...
		/************ Start operation ************/
		$q = "
        INSERT IGNORE INTO ms_config(site_id,config_name,`config_value`,`status`,`chamber_sync_flag`) VALUES";
        ...
        // if (!empty($post)) {
            ...
            // foreach ($post as $key => $rs) {
                ...
                // debug($rs['edit'],1);
                if (isset($attr['site_id'])) $site_id = replace_quote($attr['site_id']);
                if (isset($attr['config_name'])) $config_name = replace_quote($attr['config_name']);
                if (isset($attr['config_value'])) $config_value = replace_quote($attr['config_value']);
                // if (isset($rs['config_value'])) $created_at = get_datetime();
                

                $q.= " (" . $site_id . "," . $config_name . "," . $config_value . ", 1 , 10)";
        ...
        $q.= " ON DUPLICATE KEY UPDATE `config_value` = VALUES(`config_value`), updated_at = now(), chamber_sync_flag = 10;
        ";
