...
class SiteController extends ApiController {
    ...
    */
	...
	public function get()
	{
		...
		$data = orm_get($q);
		$temp = explode(" ",$data->start_date_counting);
		$date = explode("-", $temp[0]);
		$time = explode(":", $temp[1]);
		unset($temp);
		// date = year-month-day
		$temp['date'] = $date[2]."-".$date[1]."-".$date[0];
		// time = hour:minute
		$temp['time'] = $time[0].":".$time[1];
		$data->start_date_counting = $temp['date']." ".$temp['time'];
	...
	public function get_list_dropdown()
	{
		...
		if (! empty($_GET)) $attr = $_GET;
		$q = '
        SELECT site_id, site_name FROM ' . $this->table . ' 
	...
	public function get_sales_order_per_site(){
		// $attr = $result = NULL;
		// if (! empty($_GET)) $attr = $_GET;
		$q = "SELECT site_id, SUM(qty) AS qty FROM tr_transaction GROUP BY site_id";
		$data = orm_get_list($q);
		if (empty($data)) $data = NULL;
        $result = $data;
    ...
    // show TRUE / FALSE reset days 
	public function get_reset_day()
	{
		$attr = $result = NULL;
		if (! empty($_GET)) 
			$attr = $_GET;
