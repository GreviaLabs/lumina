...
class MovementTypeController extends ApiController {
	...
	|
    */
    public $table = 'ms_movement_type';
    public $primary_key = 'movement_type_id';
	public $list_column = array('movement_type_id', 'movement_type_desc','status', 'created_at', 'created_by','created_ip','updated_at','updated_by','updated_ip');
	...
	public function get_list()
	{
		...
		$q = 'SELECT mt.movement_type_id, mt.movement_type_desc, mt.created_at, mt.updated_at, mt.status,';
		$q.= ' COALESCE((SELECT u.user_code FROM ms_user u WHERE mt.created_by = u.user_id), mt.created_by) as creator,';
		$q.= ' COALESCE((SELECT u.user_code FROM ms_user u WHERE mt.updated_by = u.user_id), mt.updated_by) as editor';
		$q.= ' FROM ' . $this->table . ' mt WHERE 1';
