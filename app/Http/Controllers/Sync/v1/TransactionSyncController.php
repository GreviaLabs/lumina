...
class TransactionSyncController extends SyncController {
    ...
    */
    ...
    public $list_column = array('transaction_id','site_id','user_id','article','movement_type','customer_article','description','qty','value','status_in_out','reason_id','wo_wbs','remark','chamber_sync_flag', 'field_sync', 'status', 'created_at', 'created_by','created_ip','updated_at','updated_by','updated_ip');
	...
	// for sync with array
	public function save_bulk_array()
	{
		$post = $attr = $result = NULL;
		if (! empty($_POST)) $post = $_POST;
	...
	public function save_bulk()
	{
		...
		if (! empty($post)) {
			for($i=0; $i<count($post); $i++){
				...
				// deprecated 02/10/2019 by Harvei
				// chamber changes format to Y:m:d H:i:s
				// if(!empty($post[$i]['picktime'])){
				// 	$datetime = explode(" ", $post[$i]['picktime']);
				// 	// $datetime will be an array of 3 [date, time, AM/PM]
				// 	$post[$i]['picktime'] = $this->convert_datetime($datetime);
				// }
				// if(!empty($post[$i]['created_at'])){
				// 	$datetime = explode(" ", $post[$i]['created_at']);
				// 	// $datetime will be an array of 3 [date, time, AM/PM]
				// 	$post[$i]['created_at'] = $this->convert_datetime($datetime);
				// }
				// if(!empty($post[$i]['updated_at'])){
				// 	$datetime = explode(" ", $post[$i]['updated_at']);
				// 	// $datetime will be an array of 3 [date, time, AM/PM]
				// 	$post[$i]['updated_at'] = $this->convert_datetime($datetime);
				// }
	...
	// function for convert datetime value from .Net (JSON Chamber) (m/d/Y G:i:s A) to MySql type (Y-m-d H:i:s)
	public function convert_datetime($datetime){
		// date
		if(strpos($datetime[0],"/") !== false){
			// m/d/y convert to d/m/y first
