...
class ReasonTypeSyncController extends SyncController {
    ...
    */
    ...
    public $list_column = array('article_id','site_id','attribute','attribute_value','status','chamber_sync_flag','field_sync','last_sync', 'created_at', 'created_by','created_ip','updated_at','updated_by','updated_ip');
	...
	{
		$post = $attr = $result = $param_where = $post_reason_type_id = NULL;
        ...
        if (! isset($post['reason_type_id'])) $result['message'][] .= 'reason_type_id must be filled';
		...
		$q = "UPDATE " . $this->table;
		$q.= " SET " . $list_param;
		$q.= " WHERE " . $this->primary_key . " AND reason_type_id = " . replace_quote($post['reason_type_id']);
